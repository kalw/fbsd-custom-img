...
ALL_FLAVORS=$(get_all_latest_releases_list)
...
FLAVORS=${FREEBSD_FLAVORS:-"${ALL_FLAVORS}"}
...
for FREEBSD_ARCH  in $(echo ${ARCHS} | tr ',' ' '); do
  ...
  for FREEBSD_VERSION in $(echo ${VERSIONS} | tr ',' ' '); do
    ...
    curl -C - -o ${ISO_DL_PATH}/${FREEBSD_IMG_NAME}.xz "${FREEBSD_ISO_URL}.xz" 
    xz -d ${ISO_DL_PATH}/${FREEBSD_IMG_NAME}.xz
    for FREEBSD_FLAVOR in $(echo $FLAVORS | tr ',' ' '); do
      mdconfig -u 0 -f ${ISO_DL_PATH}/${FREEBSD_IMG_NAME}
      ...
      mdconfig -du 0
      mkdir -p artifacts 
      cp ${ISO_DL_PATH}/${FREEBSD_IMG_NAME} artifacts/${FREEBSD_FLAVOR}-${FREEBSD_IMG_NAME}
    done
  done
done
xz artifacts/*.img
